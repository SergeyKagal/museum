...
burgerBtn.onclick = function () {
  ...
  if (window.innerWidth < 768) {
    if (switcher > 0) {
      ...
      switcher *= -1;
      document
        .querySelector('.header-menu-420-inner')
        .classList.remove('header-menu-420-inner-show');
...
function muteToggle() {
  ...
  if (player.volume) {
    ...
    sound__range.style.background = `linear-gradient(to right, #710707 0%, #710707 ${
      vol * 100
    }%, #fff ${vol * 100}%, white 100%)`;
  }
}
...
document.addEventListener('keypress', (ev) => {
  ...
  if (ev.code == 'Space') {
    ...
    player.playbackRate += 0.1;
  } else if (ev.code == 'KeyF') player.requestFullscreen();
});
